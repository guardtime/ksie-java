 ...
 */
public class SignatureHandler extends ContentHandler<EnvelopeSignature> {
    ...
    @Override
    protected EnvelopeSignature getEntry(String name) throws ContentParsingException {
        try (InputStream stream = fetchStreamFromEntries(name)) {
            EnvelopeSignature read = signatureFactory.read(stream);
