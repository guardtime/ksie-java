...
public class EnvelopeCloseableIntegrationTest extends AbstractCommonIntegrationTest {
  ...
  public void testWriteEnvelopeWithTempFileRemoved() throws Exception {
      ...
      try (
          ...
          // Magic number = 1 document + 1 annotation per signature content + 1 unknown file for ENVELOPE_WITH_MULTIPLE_SIGNATURES
  ...
  public void testCreateEnvelopeFromExistingAndAlteredTempFile_ThrowsInvalidPackageException() throws Exception {
      ...
      try (
          ...
          List<File> tempFiles = getKsieTempFiles();
          //Magic number 3 = 1 document + 1 annotation per signature content for ENVELOPE_WITH_NON_REMOVABLE_ANNOTATION
          //               + 1 for new document since DocumentBuilder uses TemporaryFileParsingStore.
  ...
  private boolean anyKsieTempFiles() {
      return getKsieTempFiles().size() >= 1;
  }
  ...
  private List<File> getKsieTempFilesFromDirectory(File dir) {
    List<File> ksieTempFiles = new ArrayList<>();
  ...
  private List<File> tempFiles(File dir) {
  ...
  private boolean isTempDir(File s) {
